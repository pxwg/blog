...
$
  S = frac(1, 2pi) integral_(X) b overline(partial) c,
$
where $b in Gamma(X, L^(times.o lambda))$, $c in Gamma(X, K times.o L^(- times.o lambda))$, $L$ is a holomorphic line bundle and $K$ is the canonical bundle over $X$.
...
$
  T(z)J(w) = frac(1 - 2 lambda, (z - w)^(3)) dif z + frac(J(w), (z-w)^(2)) + frac(partial_(z) J(w), z- w) + :T(z)J(w):,
$
which implies the current changing while one consider the conformal transformation $z |-> w$:
$
  J(w) = J'(z) + frac(1-2 lambda, 2) partial (ln partial_(z) w) = J'(z) + frac(1 - 2 lambda, 2) dif (ln partial_(z) w).
$
...
The conserved quantity here from classical mechanics could be naively written as $integral_(Sigma) overline(partial) J dif x =0$.
...
While we want to glue patches into one dimensional complex manifold, a holomorphic function $f$ satisfies $partial_(z) f(p) != 0$ would play a role as _transition function_, which indeed is a conformal transformation $z |-> w$.
...
$
  integral_(X) overline(partial) J := sum_(i) integral_(U_(i)) overline(partial) J(z_(i)).
$
...
$
  integral_(X) overline(partial) J := sum_(i) integral_(U_(i)) dif J(z_(i)).
$
...
$
  J(z_(j)) = J(z_(i)) + frac(1 - 2 lambda, 2) dif (ln partial_(z_(i)) f_(i j)).
$
...
One can embed the consideration above into _Čech complex_, where $J(z_(i))$ is an element in $C^(0)(cal(U), Omega^(1))$, and $frac(1 - 2 lambda, 2) dif (ln partial_(z_(i)) f_(i j))$ is an element in $C^(1)(cal(U), Omega^(1))$, where:
...
$
  delta J_(i j) = frac(1 - 2 lambda, 2) dif (ln partial_(z_(j)) f_(i j)),
$
    ...
    theme => {
      ...
      let it = [$
        #align(center, diagram(
          ...
          & edge("u", dif, ->) frac(1-2 lambda, 2) ln partial_(z_(j)) f_(i j),$,
        ))\
        quad
      $]
...
$
  partial_(z_(j)) f_(i j) partial_(z_(k)) f_(j k) partial_(z_(i)) f_(k i) = 1 := e^(2 i pi n_(i j k)),
$
we have:
    ...
    theme => {
      ...
      let it = [$
        #align(center, diagram(
          cell-size: (1mm, 1mm),
          $J_(i) edge(delta, ->) & delta J_(i j) \
          & edge("u", dif, ->) frac(1-2 lambda, 2) ln partial_(z_(j)) f_(i j) edge(delta, ->) & (1-2 lambda) i pi n_(i j k) \
...
Note that our integration is over $X$ for $overline(partial) J$, then we need to include $overline(partial) J = dif J$ into the consideration, thus we have:
...
Using the diagram above, we could replace the ill-defined integration of $overline(partial) J$ over $X$ by the well-defined integration of $dif J_(i)$ over each patch $U_(i)$, then by the globally well-defined $2$-form $omega in Omega^(2)(X)$.
...
$
  integral_X omega = sum_({e_(i j)}) integral_(e_(i j)) (delta J)_(i j) = sum_({e_(i j)}) integral_(e_(i j)) frac(1 - 2 lambda, 2) dif (ln partial_(z_(j)) f_(i j)),
$
...
$
  integral_(X) omega = (1 - 2 lambda) i pi chevron.l [n], [X] chevron.r,
$
...
Therefore, the integral of $overline(partial) J$ (in fact, $omega$) over Riemann surface $X$ gives
$
  integral_(X) overline(partial) J := integral_(X) omega = (1 - 2 lambda) pi i c_(1)(L),
$
...
$
  overline(partial) c = 0, quad overline(partial) b = 0.
$
We denote the number of zero modes for $b$, $c$ fields as $B$ and $C$ respectively.
It is easy to identify that $C = ker(overline(partial)_(K times.o L^(-lambda)))$ and $B = ker(overline(partial)_(L^(lambda)))$, thus the difference of the zero modes is given by:
$
  C - B = dim(H^(0)(X, cal(O)(K times.o L^(-lambda)))) - dim(H^(0)(X, cal(O)(L^(lambda)))),
$
using Serre duality $H^(i)(X, cal(O)(K times.o L^(-lambda))) approx H^(n-i)(X, cal(O)(L^(lambda)))^(or)$, one have (in our case, $n=1$):
$
  C - B = dim(H^(1)(X, cal(O)(L^(lambda)))) - dim(H^(0)(X, cal(O)(L^(lambda)))) := h^(1)(X, cal(O)(L^(lambda))) - h^(0)(X, cal(O)(L^(lambda))),
$
thus the index of elliptic operator $overline(partial)$ could be rephrased as:
$
  "ind"(overline(partial)) = h^(0)(X, cal(O)(L^(lambda))) - h^(1)(X, cal(O)(L^(lambda))),
$
...
$
  Q := frac(1, 2 pi i)integral_(X) overline(partial) J(z) = frac(1, 2 pi i) integral.cont :b(z) c(z):,
$
We can use the path integral to evaluate this charge, which we have:
$
  chevron.l Q chevron.r = frac(1, Z) integral d mu thin Q e^(-S[b,c]),
$
...
$
  chevron.l Q chevron.r := frac(integral d mu thin Q cal(O)[b,c] e^(-S), integral d mu thin cal(O)[b,c] e^(-S)),
$
...
$
  bra(0)T{[Q, O(z)] ...}ket(0) = chevron.l (Q(C_1) - Q(C_2) O(z) ...) chevron.r.
$
...
$
  chevron.l Q chevron.r = B - C,
$
which is independent of the choice of $cal(O)$.
Thus, we have identified the index of elliptic operator $overline(partial)$ with the $U(1)$ charge:
$
  chevron.l Q chevron.r = "ind"(overline(partial)).
$
...
$
  chevron.l Q chevron.r = frac(1 - 2 lambda, 2) chi(L) = (1 - 2 lambda)(1-g) := "deg"(L^(lambda))+ 1- g,
$
...
$
Using the index expression $"ind"(overline(partial)) = h^(0)(X, cal(O)(L^(lambda))) - h^(1)(X, cal(O)(L^(lambda)))$, this is precisely the Riemann-Roch theorem:
$
...
// $
//   Y(T, z)Y(J, w) = frac(1 - 2 lambda, (z - w)^(3)) dif z + frac(Y(J, w), (z-w)^(2)) + frac(partial_(z) Y(J,w), z- w) + :Y(T,z) Y(J, w): ... thin ,
// $
// 我们省略了一些不必要的 OPEs.
