...
#remark[
  ...
  $
  and Wick contraction, we could rewrite the expression above as $ chevron.l phi_1 phi_2 phi_3 phi_4 chevron.r_(M oo^(p)_(q) N) = sum_(i) C_(12)^(i) C_34^(i) cal(F)_(i)(q), $
...
$
  phi_(i) times phi_(j) = sum_(k) N_(i j)^(k) phi_(k) -->^"State-Field" V_(h_(i)) times.o V_(h_(j)) => sum_(k) N_(i j)^(k) V_(h_(k)),
$
...
#proof[
  ...
  *Linear independence.*
  ...
  Since the elements
  $(psi^(12)_p)_alpha times.o (psi^34_(p))_beta$ are linearly independent in the
...
$
where the matrix $F_(p q)mat(j, k; i, l): V^(i)_(q l) times.o V^(q)_(j k) -> V^(i)_(j p) times.o V^(p)_(k l)$ is called the _F-matrix_, and column $[j, k]$ denotes the fusion of modules $V_(h_(j))$ and $V_(h_(k))$, and row $[i, l]$ denotes the initial modules $V_(h_(i))$ and outer module $V_(h_(l))$.
...
Thus, though the cutting are different, both of them give a basis of the same space of conformal blocks $cal(B)_(1,1)(V_(h_(i))):= plus.o.big_(j in cal(J)) V^(i)_(i j)$, thus the only difference is the change of basis, which could be written as:
$
...
$
where the matrix $B_(p q)mat(j, k; i, l): V^(i)_(j p) times.o V^(p)_(k l) -> V^(i)_(j q) times.o V^(q)_(k l)$ is called the _braiding matrix_.
