...
$
其中 $b in Gamma(X, L^(times.o lambda))$，$c in Gamma(X, K times.o L^(- times.o lambda))$，$L$ 是一个全纯线丛，$K$ 是 $X$ 上的典范丛。
...
从经典力学角度看，这里的守恒量可以朴素地写为 $integral_(Sigma) overline(partial) J dif x =0$。这个方程在 $CC$ 上对于具有紧支撑的 $b c$ 场是成立的。然而，当我们考虑该场论的全局结构时，需要进行更精确的分析。

== 从局域数据粘合坐标卡

当我们将坐标卡粘合成一个一维复流形时，满足 $partial_(z) f(p) != 0$ 的全纯函数 $f$ 将扮演 _转移函数_ 的角色，它实际上就是一个共形变换 $z |-> w$。因此，回顾引言中的讨论，$overline(partial) J$ 在 $X$ 上的积分应该被重新表述为在由某些共形变换粘合的多个坐标卡上的积分之和：
$
...
我们可以将上述考虑嵌入到 _Čech 复形_ 中，其中 $J(z_(i))$ 是 $C^(0)(cal(U), Omega^(1))$ 中的一个元素，而 $frac(1 - 2 lambda, 2) dif (ln partial_(z_(i)) f_(i j))$ 是 $C^(1)(cal(U), Omega^(1))$ 中的一个元素，这里：
...
其中 $dif$ 的作用相当于一个嵌入 $H^(2)(X, ZZ) arrow.hook Omega^(0)(U_(i j k))$，即对于 $[n] in H^(2)(X, ZZ)$ 有 $dif: n |-> n_(i j k)$。注意到我们的积分是对 $overline(partial) J$ 在 $X$ 上进行的，因此我们需要将 $overline(partial) J = dif J$ 纳入考虑，于是我们有：
...
利用上图，我们可以将 $overline(partial) J$ 在 $X$ 上不明确的积分，替换为 $dif J_(i)$ 在每个坐标卡 $U_(i)$ 上的良定义积分，进而替换为全局良定义的 2-形式 $omega in Omega^(2)(X)$ 的积分。
...
因此，$overline(partial) J$（实际上是 $omega$）在黎曼面 $X$ 上的积分给出：
$
...
$
我们分别用 $B$ 和 $C$ 表示 $b, c$ 场的零模个数。不难发现 $C = ker(overline(partial)_(K times.o L^(-lambda)))$ 且 $B = ker(overline(partial)_(L^(lambda)))$，因此零模数之差为：
$
  C - B = dim(H^(0)(X, cal(O)(K times.o L^(-lambda)))) - dim(H^(0)(X, cal(O)(L^(lambda)))),
$
利用 Serre 对偶 $H^(i)(X, cal(O)(K times.o L^(-lambda))) approx H^(n-i)(X, cal(O)(L^(lambda)))^(or)$，我们得到（在我们的情况下，$n=1$）：
$
  C - B = dim(H^(1)(X, cal(O)(L^(lambda)))) - dim(H^(0)(X, cal(O)(L^(lambda)))) := h^(1)(X, cal(O)(L^(lambda))) - h^(0)(X, cal(O)(L^(lambda))),
$
因此椭圆算子 $overline(partial)$ 的指标可以重述为：
$
...
$
这与 $cal(O)$ 的选择无关。因此，我们将椭圆算子 $overline(partial)$ 的指标与 $U(1)$ 荷等同起来：
$
...
$
使用指标表达式 $"ind"(overline(partial)) = h^(0)(X, cal(O)(L^(lambda))) - h^(1)(X, cal(O)(L^(lambda)))$，这正是 Riemann-Roch 定理：
$
